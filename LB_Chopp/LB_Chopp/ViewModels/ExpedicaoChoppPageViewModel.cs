...
{
    public class ExpedicaoChoppPageViewModel : ViewModelBase
    {
        Cliente _cliente;
        ...
        public DelegateCommand<ReservaChopp> ExpedirCommand { get; }
        public DelegateCommand<ReservaChopp> AnexarFotoCommand { get; }

        readonly IPageDialogService dialogService;
        readonly IDataService dataService;
        public ExpedicaoChoppPageViewModel(INavigationService navigationService, IPageDialogService _dialogService, IDataService _dataService)
            : base(navigationService)
        {
            ...
            ExpedirCommand = new DelegateCommand<ReservaChopp>(async (ex) =>
            {
                if (ex != null)
                {
                    NavigationParameters param = new NavigationParameters();
                    param.Add("RESERVA", ex);
                    await NavigationService.NavigateAsync("ExpedirPage", param);
                }
            });
        ...
        public async Task BuscarReservasAsync()
        {
            using (UserDialogs.Instance.Loading(title: string.Empty, maskType: MaskType.Black))
            {
                try
                {
                    ...
                    await dataService.GetReservaExpedirAsync(string.IsNullOrWhiteSpace(Id_reserva.SoNumero()) ? 0 : int.Parse(Id_reserva.SoNumero()),
